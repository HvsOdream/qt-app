...
import { signOut, signInWithGoogle, signInWithEmail, getUser, onAuthStateChange } from '@/lib/supabase';
      ...
      <div className="w-full max-w-sm">
        ...
        {loginSent ? (
          <div className="bg-white/10 rounded-2xl p-6 text-center">
            ...
            <p className="text-white font-bold mb-1">메일을 확인해줘!</p>
            <p className="text-blue-200 text-sm">{loginEmail}로 로그인 링크를 보냈어. 링크 클릭하면 바로 입장!</p>
            <button onClick={() => setLoginSent(false)} className="mt-4 text-blue-300 text-xs underline">다시 보내기</button>
          </div>
        ) : (
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 bg-white text-gray-800 font-bold py-4 rounded-2xl shadow-lg active:scale-95 transition-transform disabled:opacity-50">
            ...
            <div className="bg-white/10 rounded-2xl p-4">
              <p className="text-blue-200 text-xs mb-2.5 font-medium">이메일로 로그인 (비밀번호 없음)</p>
              <input
                type="email"
                value={loginEmail}
                ...
                onKeyDown={e => {
                  if (e.key === 'Enter' && loginEmail) {
                    ...
                    signInWithEmail(loginEmail).then(() => { setLoginSent(true); setLoginLoading(false); });
                  }
                }}
                placeholder="이메일 주소 입력"
                className="w-full bg-white/20 text-white placeholder-blue-300 rounded-xl px-3 py-2.5 text-sm outline-none focus:bg-white/30 mb-2"
              />
              <button
                onClick={async () => {
